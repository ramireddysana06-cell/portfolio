...
Let me introduce myself, I'm **Sana Rami Reddy**. On this occasion, I'd like to share the portfolio website project that I've developed.

## 🚀 Live Demo

**Website Link:** [https://portofolio-v5-git-main-sana-rami-reddy.vercel.app/]
...
```bash
git clone https://github.com/sana-rami-reddy/Portofolio_V5.git
cd Portofolio_V5
```
...
> **Note:** while Supabase is the primary source for certificates, the frontend also includes a built-in fallback that bundles any images placed in the `certificates/` directory at the project root. These local files are used automatically when the database has no rows or when you are running the app offline. For a production build you can move these images into `public/certificates` or upload them to Supabase and update the `certificates` table accordingly.
...
-- you can substitute the URL with any publicly accessible link, or omit the database entirely and rely on the bundled local images mentioned above.

-- Insert one example comment
INSERT INTO public.portfolio_comments (content, user_name) 
VALUES ('Created By Sana Rami Reddy', 'Sana');
