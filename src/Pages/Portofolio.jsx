...
// techStacks updated
const techStacks = [
  { icon: "/python.svg", language: "Python Programming" },
...
export default function FullWidthTabs() {
  ...
  // local fallback images in case no data from Supabase
  ...
  const fetchData = useCallback(async () => {
    try {
      // Mengambil data dari Supabase untuk projects
      ...
      // Mengambil data dari Supabase untuk certificates
      ...
      if (certificateData.length > 0) {
        ...
        console.warn("No certificates returned from Supabase, keeping local fallback.");
      }
  ...
  useEffect(() => {
    ...
    if (cachedCertificates) {
        // if local storage already has data, use it
        const parsed = JSON.parse(cachedCertificates);
        if (parsed.length > 0) setCertificates(parsed);
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 md:gap-5 gap-4">
                {displayedCertificates.map((certificate, index) => (
                  <div
                    ...
                    data-aos-duration={index % 3 === 0 ? "1000" : index % 3 === 1 ? "1200" : "1000"}
                  >
                    <Certificate ImgSertif={certificate.Img} Title={certificate.Title} Description={certificate.Description} />
                  </div>
                ))}
              </div>
            </div>
